...
\]
where $\lambda^{(v)}$ are the eigenvalues and $\bf x^{(v)}$ the corresponding eigenvectors.\footnote{The introduction to this section is based on the lecturenotes"lectures2015.pdf"} 

When the matrix is $\bf A$ is real and symetric the Jacobi rotation algoritm can be used to solve the eigenvalue problem. Let $\bf D$ be the diagonal matrix with the eigenvalues of $\bf A$ on the diagonal:

\begin{equation}
    \left( \begin{array}{ccccccc} \lambda_1 & 0 & 0   & 0    & \dots  &0     & 0 \\
...
\]
where $\bf{S_N\dots S_1} = \bf S'$ must be true. This must be done on both side of the equation. Only one transformation is given by:
\begin{align*}
...
\end{align*}
where we want to choose the angle $\theta$ so that all the non-diagonal matric elements becomes zero, $b_{kl} = 0$. Introducing a new variable $\tau$:
\begin{align*}
b_{kl} &= cs(a_{kk} - a_{ll}) + (c^2 - s^2)a_{kl} = 0\\
\frac{a_{ll}-a_{kk}}{a_{kl}} &= \frac{c^2 - s^2}{cs} = \frac{2\cos{2\theta}}{\sin{2\theta}} = 2 \cot{2\theta}\\
\tau &= \cot{2\theta} = \frac{a_{ll}-a_{kk}}{2a_{kl}}
\end{align*}
Then $b_{kl} = 0$ can be written as:
\begin{align*}
2\tau &= \frac{c^2 - s^2}{cs} = \frac{c^2}{cs} - \frac{s^2}{cs} = \frac{1}{t} - t\\
2\tau t &= 1 - t^2 \Rightarrow t^2 +2\tau t - 1 = 0\\
\end{align*}
with the solution
...
\]
We then define the angle $\theta$ so that the non-diagonal matrix elements of the transformed matrix 
...
\section{Physics 1: One electron in a three-dimentional harmonic oscillator potential.}
\subsection{Rewriting the Schr\"odinger's equation to a dimentionaless form}
...
Then we use the substitution $R(r) = (1/r) u(r)$ and obtain
\[
...
\noindent\rule{\textwidth}{1pt}
\section*{Project 2, Schr\"odinger's equation for two electrons in a three-dimensional harmonic oscillator well}
